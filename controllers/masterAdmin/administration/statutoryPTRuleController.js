const { validateRequest, adminCommonQuery, handleError, sequelize, Op } = require("../../../helpers");
...
exports.create = async (req, res) => {
    try {
        ...
        req.body.user_id = -1;
        await adminCommonQuery.createRecord(StatutoryPTRule, req.body, transaction, false);
...
exports.dropdownList = async (req, res) => {
    try {
        // const result = await adminCommonQuery.findAllRecords(StatutoryPTRule, { status: 0 });
        const fieldConfig = [
            ["min_salary", true, true],
            ["max_salary", true, true],
            ["monthly_amount", true, true],
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        req.body.branch_id = -1;
        req.body.user_id = -1;
        const updated = await adminCommonQuery.updateRecordById(StatutoryPTRule, req.params.id, req.body, transaction, false, false);
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        const deleted = await adminCommonQuery.softDeleteById(StatutoryPTRule, ids, transaction, false);
        if (!deleted) {
            await transaction.rollback();
...
exports.calculatePT = async (req, res) => {
    try {
        ...
        if (employee_id) {
            const employee = await adminCommonQuery.findOneRecord(Employee, { id: employee_id }, null, false, {});
            if (employee) {
        ...
        if (!state_id && state_name) {
            const state = await adminCommonQuery.findOneRecord(StateMaster, { state_name: { [Op.iLike]: state_name } }, null, false, {});
        ...
        // 3. Find matching rules using adminCommonQuery
        const filters = {
            state_id,
            status: 0,
        ...
        const matchingRules = await adminCommonQuery.findAllRecords(StatutoryPTRule, filters, {}, null, false);
