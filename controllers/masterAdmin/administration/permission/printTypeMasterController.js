...
exports.create = async (req, res) => {
  ...
  try {
    await adminCommonQuery.createRecord(PrintTypeMaster, req.body, transaction);
...
exports.getAll = async (req, res) => {
  try {
    const result = await adminCommonQuery.findAllRecords(PrintTypeMaster, { status: 0 });
...
exports.getById = async (req, res) => {
  try {
    const record = await adminCommonQuery.findOneRecord(PrintTypeMaster, req.params.id);
...
exports.update = async (req, res) => {
  ...
  try {
    const updated = await adminCommonQuery.updateRecordById(PrintTypeMaster, req.params.id, req.body, transaction);
...
exports.delete = async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const deleted = await adminCommonQuery.softDeleteById(PrintTypeMaster, req.params.id, transaction);
