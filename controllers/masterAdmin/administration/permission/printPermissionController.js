...
exports.create = async (req, res) => {
  ...
  try {
    await adminCommonQuery.createRecord(PrintPermission, req.body, transaction);
...
exports.getAll = async (req, res) => {
  try {
    const result = await adminCommonQuery.findAllRecords(PrintPermission, { status: 0 });
...
exports.getById = async (req, res) => {
  try {
    const record = await adminCommonQuery.findOneRecord(PrintPermission, req.params.id);
...
exports.update = async (req, res) => {
  ...
  try {
    const updated = await adminCommonQuery.updateRecordById(PrintPermission, req.params.id, req.body, transaction);
...
exports.delete = async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const deleted = await adminCommonQuery.softDeleteById(PrintPermission, req.params.id, transaction);
