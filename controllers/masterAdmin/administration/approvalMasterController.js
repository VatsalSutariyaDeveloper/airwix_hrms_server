...
exports.createCompleteWorkflow = async (req, res) => {
    ...
    try {
        ...
        const workflowData = {
            ...
            company_id,
            user_id: user_id, // adminCommonQuery expects user_id for audit logs
            status: 1
        };
        const workflow = await adminCommonQuery.createRecord(ApprovalWorkflow, workflowData, transaction);
        ...
        if (Array.isArray(rules) && rules.length > 0) {
            ...
            await adminCommonQuery.bulkCreate(ApprovalRule, rulesData, {}, transaction);
        }
        ...
        if (Array.isArray(levels) && levels.length > 0) {
            ...
            await adminCommonQuery.bulkCreate(ApprovalLevel, levelsData, {}, transaction);
        }
...
exports.getWorkflowById = async (req, res) => {
    try {
        const { id } = req.params;
        const workflow = await adminCommonQuery.findOneRecord(ApprovalWorkflow, { id }, {
            include: [
...
exports.updateWorkflow = async (req, res) => {
    ...
    try {
        ...
        const existingWorkflow = await adminCommonQuery.findOneRecord(ApprovalWorkflow, id, {}, transaction, false, false);
        ...
        // 2. Update Header
        await adminCommonQuery.updateRecordById(ApprovalWorkflow, id, headerData, transaction);

        // 3. Sync Rules (Delete old, add new)
        if (rules && Array.isArray(rules)) {
            await adminCommonQuery.hardDeleteRecords(ApprovalRule, { workflow_id: id }, transaction, false);
        ...
        if (levels && Array.isArray(levels)) {
            await adminCommonQuery.hardDeleteRecords(ApprovalLevel, { workflow_id: id }, transaction, false);
...
exports.deleteWorkflow = async (req, res) => {
    ...
    try {
        ...
        await adminCommonQuery.hardDeleteRecords(ApprovalRule, { workflow_id: id }, transaction, false);
        await adminCommonQuery.hardDeleteRecords(ApprovalLevel, { workflow_id: id }, transaction, false);
        const deleted = await adminCommonQuery.hardDeleteRecords(ApprovalWorkflow, { id }, transaction, false);
...
exports.configureApprovalType = async (req, res) => {
    ...
    try {
        ...
        await adminCommonQuery.updateRecordById(
            ApprovalWorkflow,
        ...
        if (type === 2) {
            ...
            const wf = await adminCommonQuery.createRecord(ApprovalWorkflow, {
                module_entity_id,
            ...
            // Level 1 = Admin Role
            await adminCommonQuery.createRecord(ApprovalLevel, {
                workflow_id: wf.id,
                level_sequence: 1,
                approver_type: 'ROLE',
