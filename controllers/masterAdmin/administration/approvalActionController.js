...
const { sequelize, handleError, adminCommonQuery, Op } = require("../../../helpers");
...
exports.getMyPendingApprovals = async (req, res) => {
    try {
        ...
        const pendingRequests = await adminCommonQuery.findAllRecords(
            ApprovalRequest,
            {
                status: 'PENDING',
                company_id
        ...
        for (const req of pendingRequests) {
            ...
            const currentLevel = await adminCommonQuery.findOneRecord(
                ApprovalLevel,
                {
            ...
            if (currentLevel) {
                ...
                if (isUserMatch || isRoleMatch) {
                    ...
                    if(req.module_entity_id === MODULES.SALES.QUOTATION.ID) {
                        entityDetails = await adminCommonQuery.findOneRecord(
                            Quotation,
                            req.entity_id, 
    ...
    try {
        ...
        if (result.status === 'APPROVED') {
            ...
            if (entity_type === MODULES.SALES.QUOTATION.ID) {
                await adminCommonQuery.updateRecordById(Quotation, entity_id, { status: 0 }, transaction);
            } else if (entity_type === MODULES.SALES.SALES_ORDER.ID) {
                await adminCommonQuery.updateRecordById(SalesOrder, entity_id, { status: 0 }, transaction);
            } 
            ...
            // Rejected
            if (entity_type === MODULES.SALES.QUOTATION.ID) {
                await adminCommonQuery.updateRecordById(Quotation, entity_id, { status: 1 }, transaction);
            } else if (entity_type === MODULES.SALES.SALES_ORDER.ID) {
                await adminCommonQuery.updateRecordById(SalesOrder, entity_id, { status: 1 }, transaction);
            } 
        }
...
exports.getApprovalHistory = async (req, res) => {
    try {
        ...
        const requests = await adminCommonQuery.findAllRecords(ApprovalRequest, {
        ...
        const logs = await adminCommonQuery.findAllRecords(ApprovalLog, { 
...
exports.createApprovalRequest = async (req, res) => {
    ...
    try {
        ...
        recordData = await adminCommonQuery.findOneRecord(Model, entity_id, {}, transaction, false, false);
        ...
        const existingRequest = await adminCommonQuery.findOneRecord(ApprovalRequest, {
            module_entity_id: entity_type,
            entity_id: entity_id,
            status: 'PENDING'
