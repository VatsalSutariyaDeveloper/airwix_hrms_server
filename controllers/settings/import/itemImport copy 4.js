...
console.log('fileDerivedParentNames:', Array.from(fileDerivedParentNames));
    // --- 3. FETCH MASTERS ---
    ...
    for (const group of taxGroups) {
        ...
        console.log(`rate=${rate}, isNaN=${isNaN(rate)}`);
...
console.log('Built taxGroupMap:', masterData.taxGroupMap);
    ...
    // ✅ Store extra batches for Bulk Items
    const additionalStockList = [];
    // ✅ Map to track IDs of variants created Linearly
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        console.log("firstisVariant",isVariant);
        ...
        if (batchWise === 1 && serialNoWise === 1) fail("Cannot enable both Batch Wise and Serial Number Wise.");
        ...
        console.log("Resolved tax rate:", rate, "intraTaxId:", intraTaxId);
        ...
        const codeRaw = String(seriesCode || '').trim().toLowerCase();
        const isAuto = !seriesCode || codeRaw === '' || codeRaw === 'auto';
        ...
        console.log("isVariantbefore",isVariant);
        console.log("fileDerivedParentNamesBefore",fileDerivedParentNames.has(workingItemNameLower));
        if (!isVariant && fileDerivedParentNames.has(workingItemNameLower)) {
            console.log('openingStock',openingStock);
             if (parseFloat(openingStock) > 0) {
                console.log("isVariantIn",isVariant,fileDerivedParentNames);
            ...
            if (isDuplicateItem) {
                ...
                if (existingParent && existingParent.id) {
                     ...
                     if (maintainStock === 1 && openingStock > 0) {
                         const batchNo = batchWise === 1 ? (record.batch_no || generateBatchNo(existingParent.series_code)) : null;
