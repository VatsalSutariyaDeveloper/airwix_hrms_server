    ...
    try {
        ...
        if (!req.body.only_template) {
            ...
            if (employeesToSync.length > 0) {
                ...
                const existingCategoriesForMeta = await commonQuery.findAllRecords(LeaveTemplateCategory, { leave_template_id: id }, {}, transaction);
                const oldMasterCategoriesMap = new Map();
                existingCategoriesForMeta.forEach(c => oldMasterCategoriesMap.set(c.id, c));

                await LeaveBalanceService.bulkSyncEmployeeBalances(employeeIds, id, transaction, { oldMasterCategoriesMap });
            }
        }
