module.exports = (sequelize, DataTypes) => {
  const AttendanceTemplate = sequelize.define("AttendanceTemplate", {
    ...
    punch_confirmation_mode: { type: DataTypes.STRING(10), defaultValue: 'AUTO', validate: { isIn: [['AUTO', 'MANUAL']] }, comment: 'AUTO: punch is submitted immediately on face capture. MANUAL: employee must confirm Punch In/Out after the face capture (applies to FACE_RECOGNITION mode)' },
    ...
    comp_off_min_working_mins: { type: DataTypes.INTEGER, defaultValue: 0, comment: 'Min working mins for half Comp-Off Leave on holiday/weekly off' },
    comp_off_max_working_mins: { type: DataTypes.INTEGER, defaultValue: 0, comment: 'Min working mins for full Comp-Off Leave on holiday/weekly off' },
    ...
    allow_canteen_access: { type: DataTypes.BOOLEAN, defaultValue: false, comment: 'If true, employee has access to canteen' },
    min_canteen_days: { type: DataTypes.INTEGER, defaultValue: 10, comment: 'Minimum canteen days required for deduction' },
