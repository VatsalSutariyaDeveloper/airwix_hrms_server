/**
 * AUTO-GENERATED MODULE & ENTITY CONSTANTS
 * Generated on: 18/3/2026, 1:45:08 am
 * Usage: 
 * const { ENTITIES } = require('./moduleEntitiesConstants');
 * console.log(ENTITIES.ITEM_MASTER_ID);
 */

exports.MODULES = {
  "DASHBOARD": {
    "ID": 1,
    "NAME": "Dashboard",
    "ANALYTICS_DASHBOARD": {
      "ID": 1,
      "NAME": "Analytics Dashboard",
      "MODULE_ID": 1
    }
  },
  "ATTENDANCE": {
    "ID": 2,
    "NAME": "Attendance",
    "ATTENDANCE": {
      "ID": 61,
      "NAME": "Attendance",
      "MODULE_ID": 2
    }
  },
  "EMPLOYEE": {
    "ID": 12,
    "NAME": "Employee",
    "EMPLOYEES": {
      "ID": 60,
      "NAME": "Employees",
      "MODULE_ID": 12
    }
  },
  "CANTEEN": {
    "ID": 15,
    "NAME": "Canteen",
    "CANTEEN": {
      "ID": 80,
      "NAME": "Canteen",
      "MODULE_ID": 15
    }
  },
  "PAYROLL": {
    "ID": 3,
    "NAME": "Payroll",
    "EMPLOYEE_ADVANCE": {
      "ID": 75,
      "NAME": "Employee Advance",
      "MODULE_ID": 3
    },
    "PAYROLL_LIST": {
      "ID": 70,
      "NAME": "Payroll List",
      "MODULE_ID": 3
    },
    "INCENTIVE_TYPE": {
      "ID": 72,
      "NAME": "Incentive Type",
      "MODULE_ID": 3
    },
    "CASH_VOUCHER": {
      "ID": 81,
      "NAME": "Cash Voucher",
      "MODULE_ID": 3
    },
    "PAY_SLIP": {
      "ID": 71,
      "NAME": "Pay Slip",
      "MODULE_ID": 3
    },
    "PAYROLL_SUMMARY": {
      "ID": 83,
      "NAME": "Payroll Summary",
      "MODULE_ID": 3
    },
    "PAYMENT_HISTORY": {
      "ID": 76,
      "NAME": "Payment History",
      "MODULE_ID": 3
    },
    "EMPLOYEE_INCENTIVE": {
      "ID": 74,
      "NAME": "Employee Incentive",
      "MODULE_ID": 3
    }
  },
  "REPORTS": {
    "ID": 8,
    "NAME": "Reports",
    "ESI_REPORT": {
      "ID": 100,
      "NAME": "ESI Report",
      "MODULE_ID": 8
    },
    "PF_REPORT": {
      "ID": 102,
      "NAME": "PF Report",
      "MODULE_ID": 8
    },
    "OVERTIME_REPORT": {
      "ID": 96,
      "NAME": "Overtime Report",
      "MODULE_ID": 8
    },
    "PAYSLIP_REPORT": {
      "ID": 98,
      "NAME": "Payslip Report",
      "MODULE_ID": 8
    },
    "LEAVE_REPORT": {
      "ID": 95,
      "NAME": "Leave Report",
      "MODULE_ID": 8
    },
    "REPORT": {
      "ID": 103,
      "NAME": "report",
      "MODULE_ID": 8
    },
    "ATTENDANCE_SUMMARY": {
      "ID": 94,
      "NAME": "Attendance Summary",
      "MODULE_ID": 8
    },
    "EMPLOYER_CONTRIBUTION": {
      "ID": 93,
      "NAME": "Employer Contribution",
      "MODULE_ID": 8
    },
    "CTC_BREAKDOWN_REPORT": {
      "ID": 99,
      "NAME": "CTC Breakdown Report",
      "MODULE_ID": 8
    },
    "LATE_ENTRY_REPORT": {
      "ID": 97,
      "NAME": "Late Entry Report",
      "MODULE_ID": 8
    }
  },
  "SETTINGS": {
    "ID": 9,
    "NAME": "Settings",
    "ATTENDANCE_TEMPLATE": {
      "ID": 64,
      "NAME": "Attendance Template",
      "MODULE_ID": 9
    },
    "DATA_IMPORT": {
      "ID": 35,
      "NAME": "Data Import",
      "MODULE_ID": 9
    },
    "USERS_ROLES": {
      "ID": 32,
      "NAME": "Users & Roles",
      "MODULE_ID": 9
    },
    "SHIFT_TEMPLATE": {
      "ID": 62,
      "NAME": "Shift Template",
      "MODULE_ID": 9
    },
    "COMPANY_PROFILE": {
      "ID": 30,
      "NAME": "Company Profile",
      "MODULE_ID": 9
    },
    "HOLIDAY_TEMPLATE": {
      "ID": 54,
      "NAME": "Holiday Template",
      "MODULE_ID": 9
    },
    "LEAVE_TEMPLATE": {
      "ID": 46,
      "NAME": "Leave Template",
      "MODULE_ID": 9
    },
    "DEVICE_MASTER": {
      "ID": 51,
      "NAME": "Device Master",
      "MODULE_ID": 9
    },
    "ROLES_PERMISSION": {
      "ID": 50,
      "NAME": "Roles Permission",
      "MODULE_ID": 9
    },
    "CUSTOM_FIELDS": {
      "ID": 79,
      "NAME": "Custom Fields",
      "MODULE_ID": 9
    },
    "BRANCH_MASTER": {
      "ID": 78,
      "NAME": "Branch Master",
      "MODULE_ID": 9
    },
    "DESIGNATION": {
      "ID": 73,
      "NAME": "Designation",
      "MODULE_ID": 9
    },
    "DEPARTMENT": {
      "ID": 67,
      "NAME": "Department",
      "MODULE_ID": 9
    },
    "WEEK_OFF_TEMPLATE": {
      "ID": 59,
      "NAME": "Week Off Template",
      "MODULE_ID": 9
    },
    "SALARY_TEMPLATE": {
      "ID": 65,
      "NAME": "Salary Template",
      "MODULE_ID": 9
    }
  },
  "ADMINISTRATION": {
    "ID": 10,
    "NAME": "Administration",
    "LABOUR_WELFARE_FUND_RULE": {
      "ID": 68,
      "NAME": "Labour Welfare Fund Rule",
      "MODULE_ID": 10
    },
    "PROFESSIONAL_TAX": {
      "ID": 69,
      "NAME": "Professional Tax",
      "MODULE_ID": 10
    },
    "COMPANY_SETTING_MASTER": {
      "ID": 52,
      "NAME": "Company Setting Master",
      "MODULE_ID": 10
    },
    "MODULE_ENTITY": {
      "ID": 43,
      "NAME": "Module Entity",
      "MODULE_ID": 10
    },
    "CURRENCY": {
      "ID": 47,
      "NAME": "Currency",
      "MODULE_ID": 10
    },
    "COUNTRY": {
      "ID": 48,
      "NAME": "Country",
      "MODULE_ID": 10
    },
    "HOLIDAY_MASTER": {
      "ID": 45,
      "NAME": "Holiday Master",
      "MODULE_ID": 10
    },
    "MODULE_MASTER": {
      "ID": 42,
      "NAME": "Module Master",
      "MODULE_ID": 10
    }
  }
};

exports.ENTITIES = {
  // === ALL IDs ===
  ANALYTICS_DASHBOARD_ID: 1,
  ATTENDANCE_ID: 61,
  EMPLOYEES_ID: 60,
  CANTEEN_ID: 80,
  EMPLOYEE_ADVANCE_ID: 75,
  PAYROLL_LIST_ID: 70,
  INCENTIVE_TYPE_ID: 72,
  CASH_VOUCHER_ID: 81,
  PAY_SLIP_ID: 71,
  PAYROLL_SUMMARY_ID: 83,
  PAYMENT_HISTORY_ID: 76,
  EMPLOYEE_INCENTIVE_ID: 74,
  ESI_REPORT_ID: 100,
  PF_REPORT_ID: 102,
  OVERTIME_REPORT_ID: 96,
  PAYSLIP_REPORT_ID: 98,
  LEAVE_REPORT_ID: 95,
  REPORT_ID: 103,
  ATTENDANCE_SUMMARY_ID: 94,
  EMPLOYER_CONTRIBUTION_ID: 93,
  CTC_BREAKDOWN_REPORT_ID: 99,
  LATE_ENTRY_REPORT_ID: 97,
  ATTENDANCE_TEMPLATE_ID: 64,
  DATA_IMPORT_ID: 35,
  USERS_ROLES_ID: 32,
  SHIFT_TEMPLATE_ID: 62,
  COMPANY_PROFILE_ID: 30,
  HOLIDAY_TEMPLATE_ID: 54,
  LEAVE_TEMPLATE_ID: 46,
  DEVICE_MASTER_ID: 51,
  ROLES_PERMISSION_ID: 50,
  CUSTOM_FIELDS_ID: 79,
  BRANCH_MASTER_ID: 78,
  DESIGNATION_ID: 73,
  DEPARTMENT_ID: 67,
  WEEK_OFF_TEMPLATE_ID: 59,
  SALARY_TEMPLATE_ID: 65,
  LABOUR_WELFARE_FUND_RULE_ID: 68,
  PROFESSIONAL_TAX_ID: 69,
  COMPANY_SETTING_MASTER_ID: 52,
  MODULE_ENTITY_ID: 43,
  CURRENCY_ID: 47,
  COUNTRY_ID: 48,
  HOLIDAY_MASTER_ID: 45,
  MODULE_MASTER_ID: 42,

  // === ALL NAMES ===
  ANALYTICS_DASHBOARD_NAME: "Analytics Dashboard",
  ATTENDANCE_NAME: "Attendance",
  EMPLOYEES_NAME: "Employees",
  CANTEEN_NAME: "Canteen",
  EMPLOYEE_ADVANCE_NAME: "Employee Advance",
  PAYROLL_LIST_NAME: "Payroll List",
  INCENTIVE_TYPE_NAME: "Incentive Type",
  CASH_VOUCHER_NAME: "Cash Voucher",
  PAY_SLIP_NAME: "Pay Slip",
  PAYROLL_SUMMARY_NAME: "Payroll Summary",
  PAYMENT_HISTORY_NAME: "Payment History",
  EMPLOYEE_INCENTIVE_NAME: "Employee Incentive",
  ESI_REPORT_NAME: "ESI Report",
  PF_REPORT_NAME: "PF Report",
  OVERTIME_REPORT_NAME: "Overtime Report",
  PAYSLIP_REPORT_NAME: "Payslip Report",
  LEAVE_REPORT_NAME: "Leave Report",
  REPORT_NAME: "report",
  ATTENDANCE_SUMMARY_NAME: "Attendance Summary",
  EMPLOYER_CONTRIBUTION_NAME: "Employer Contribution",
  CTC_BREAKDOWN_REPORT_NAME: "CTC Breakdown Report",
  LATE_ENTRY_REPORT_NAME: "Late Entry Report",
  ATTENDANCE_TEMPLATE_NAME: "Attendance Template",
  DATA_IMPORT_NAME: "Data Import",
  USERS_ROLES_NAME: "Users & Roles",
  SHIFT_TEMPLATE_NAME: "Shift Template",
  COMPANY_PROFILE_NAME: "Company Profile",
  HOLIDAY_TEMPLATE_NAME: "Holiday Template",
  LEAVE_TEMPLATE_NAME: "Leave Template",
  DEVICE_MASTER_NAME: "Device Master",
  ROLES_PERMISSION_NAME: "Roles Permission",
  CUSTOM_FIELDS_NAME: "Custom Fields",
  BRANCH_MASTER_NAME: "Branch Master",
  DESIGNATION_NAME: "Designation",
  DEPARTMENT_NAME: "Department",
  WEEK_OFF_TEMPLATE_NAME: "Week Off Template",
  SALARY_TEMPLATE_NAME: "Salary Template",
  LABOUR_WELFARE_FUND_RULE_NAME: "Labour Welfare Fund Rule",
  PROFESSIONAL_TAX_NAME: "Professional Tax",
  COMPANY_SETTING_MASTER_NAME: "Company Setting Master",
  MODULE_ENTITY_NAME: "Module Entity",
  CURRENCY_NAME: "Currency",
  COUNTRY_NAME: "Country",
  HOLIDAY_MASTER_NAME: "Holiday Master",
  MODULE_MASTER_NAME: "Module Master"
};
